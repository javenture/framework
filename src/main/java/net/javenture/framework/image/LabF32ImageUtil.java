...
/*
	2022/01/20
 */
final public class LabF32ImageUtil
{
	//
	private LabF32ImageUtil()
	{
	}


	//
	public static FileContent<LabF32Image> read(File file, IImageOption... options)
	{
	...
	public static FileContent<LabF32Image> read(File file, ImageFormat format, IImageOption... options)
	{
		switch (format)
		{
			case DNG:
			{
				...
				if (option != null)
				{
					...
					return content.STATUS == FileContent.Status.OK
						? new FileContent<>(FileContent.Status.OK, ImageUtil.convert(content.VALUE, LabF32Image.INSTANCE, option.SPACE))
			...
			case JPEG:
			{
				...
				return content.STATUS == FileContent.Status.OK
					? new FileContent<>(FileContent.Status.OK, ImageUtil.convert(content.VALUE, LabF32Image.INSTANCE, RgbColorSpace.S_RGB))
					: new FileContent<>(content);
			}
	...
	public static FileContent<LabF32Image> write(LabF32Image source, File destination, IImageOption... options)
	{
	...
	public static FileContent<LabF32Image> write(LabF32Image source, File destination, ImageFormat format, IImageOption... options)
	{
		switch (format)
		{
			case JPEG:
			{
				...
				BufferedImage imageBuffered = ImageUtil.convert(source, BufferedImageUtil.INSTANCE, RgbColorSpace.S_RGB);

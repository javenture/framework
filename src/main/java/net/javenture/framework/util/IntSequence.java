...
/*
	2021/05/30
 */
final public class IntSequence                                                                                              // XXX: + IntRawSequence
	extends ASequence<Integer, IntSequence>
{
	//
	final private static int CAPACITY = 1024;                                                                                           // XXX: 1024 ?
	final public static IInstanceFactory<IntSequence> FACTORY_INSTANCE = object -> object instanceof IntSequence;
	final public static ICopyFactory<IntSequence> FACTORY_COPY = object -> object != null ? new IntSequence(object) : null;

	final public static IByteFactory<IntSequence> FACTORY_BYTE = new IByteFactory<>()
	{
		@Override
		public void toStream(@NullAllow IntSequence object, OutputStream destination)
			throws Exception
		{
			if (IByteFactory.setNullHeader(object, destination))
			{
				...
				if (count != 0)
				{
					...
					for (int i = 0; i < count; i++) IntUtil.bytes(array[i], destination);
				}
			}
		}

		@Override
		public @NullAllow IntSequence fromStream(InputStream source)
			throws Exception
		{
			if (IByteFactory.getNullHeader(source))
			{
				...
				if (count != 0)
				{
					...
					return new IntSequence(array, true);
				}
				else
				{
					return new IntSequence(IntArrayUtil.BLANK, true);
				}
			}
			else
			{
	...
	final public static IMurmur3HashFactory<IntSequence> FACTORY_MURMUR3HASH = (object, destination) ->
	{
	...
	final public static IDatabaseFactory<IntSequence, byte[]> FACTORY_DATABASE = new IDatabaseFactory<>()
	{
		@Override
		...
		@Override
		public @NullAllow byte[] toDatabase(@NullAllow IntSequence object)
		{
			if (object != null)
			{
				int count = object.size;
		...
		@Override
		public @NullAllow IntSequence fromDatabase(@NullAllow byte[] object)
			throws Exception
		{
			if (object != null)
			{
	...
	// XXX: FACTORY_MUTATION ?


	final public static Factories<IntSequence> FACTORIES = new Factories<>(FACTORY_INSTANCE, FACTORY_COPY, FACTORY_BYTE, FACTORY_MURMUR3HASH, FACTORY_DATABASE);
	...
	int size;
	@NullDisallow int[] array;

	// XXX: MUTATION (ADD, REMOVE, CLEAR)
	...
	public IntSequence(@NullDisallow IntSequence source)
	{
	...
	public IntSequence(@NullDisallow int[] array)
	{
		this(array, false);
	}


	private IntSequence(@NullDisallow int[] array, boolean wrap)
	{
	...
	int index0(int value, int from, int to)
	{
	...
	@Override
	public boolean add(@NullDisallow Integer value)
	{
	...
	private void add0(int value, int index)
	{
		ensure(1);
	...
	public void add(IntSequence sequence)
	{
		add(sequence, 0, sequence.size);
	}


	public void add(IntSequence sequence, int from, int to)
	{
		Validator.argument(from >= 0, "[from] (", from, ") is illegal");
	...
	public @NullAllow Integer min()
	{
		return size != 0 ? array[0] : null;
	}


	public @NullAllow Integer max()
	{
	...
	@Override
	public Factories<IntSequence> factories()
	{
		return FACTORIES;
	}
	...
	@Override
	public boolean equals(@NullAllow IntSequence object)
	{
		return
			this == object
			||
	...
	final public static class DatabaseField
		extends AFactoryObjectDatabaseField<IntSequence>
	{
		//
		final private Model MODEL;

		//
		public DatabaseField(String table, int number, String name, @NullAllow IntSequence init, Option... options)
		{
	...
	final public static class Model
		extends AFactoryObjectModel<IntSequence>
	{
		//
		...
		public Model(@NullDisallow String name, boolean nullable, boolean quotation, boolean trim, @NullAllow IntSequence init, @NullDisallow String... objects)
		{
		...
		@Override
		public @NullDisallow IScriptEntry toScriptEntry(@NullAllow IntSequence value)
		{
		...
		@Override
		public @NullDisallow IParser.Report<IntSequence> parse(@NullAllow CharSequence source)
		{
			return PARSER.parse(source);
		}

		@Override
		public @NullDisallow IParser.Report<IntSequence> parse(@NullAllow JsonObject source)
		{
			if (source != null)
			{
				...
				if (entry0 != null)
				{
					if (entry0.type() == IJsonEntry.Type.ARRAY)
					{
						...
						IntSequence sequence = new IntSequence(array.size());
						boolean error = false;
	...
	final public static class Parser
		implements IParser<IntSequence>
	{
		//
		final private static char DELIMITER = ',';

		//
		final private @NullAllow IntSequence INIT;
		final private boolean NULLABLE;
		final private boolean TRIM;
		final private Report<IntSequence> REPORT_NULL;

		//
		public Parser(@NullAllow IntSequence init, boolean nullable, boolean trim)
		{
		...
		//
		@Override
		public @NullAllow IntSequence init()
		{
		...
		@Override
		public Report<IntSequence> parse(@NullAllow IntSequence value)
		{
		...
		@Override
		public Report<IntSequence> parse(@NullAllow CharSequence source)
		{
			if (source != null)
			{
				if (!NULLABLE || NullUtil.notNull(source, TRIM))
				{
					if (source.length() != 0)
					{
						...
						IntSequence sequence = new IntSequence(splitter.size());
						boolean error = false;
					...
					{
						return new IParser.Report<>(true, INIT == null || INIT.size != 0, new IntSequence(0));
					}
				}
				else
				{
		...
		//
		final public static Parser BLANK = new Parser(new IntSequence(0), false, false);

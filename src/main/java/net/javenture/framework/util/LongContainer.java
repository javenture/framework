...
/*
	2021/05/30
 */
final public class LongContainer
	extends AContainer<Long, LongContainer>
{
	//
	final private static int CAPACITY = 512;
	final public static IInstanceFactory<LongContainer> FACTORY_INSTANCE = object -> object instanceof LongContainer;
	final public static ICopyFactory<LongContainer> FACTORY_COPY = object -> object != null ? new LongContainer(object) : null;

	final public static IByteFactory<LongContainer> FACTORY_BYTE = new IByteFactory<>()
	{
		@Override
		public void toStream(@NullAllow LongContainer object, OutputStream destination)
			throws Exception
		{
		...
		@Override
		public @NullAllow LongContainer fromStream(InputStream source)
			throws Exception
		{
			if (IByteFactory.getNullHeader(source))
			{
				...
				if (count != 0)
				{
					...
					return new LongContainer(array, true);
				}
				else
				{
					return new LongContainer(LongArrayUtil.BLANK, true);
				}
			}
			else
			{
	...
	final public static IMurmur3HashFactory<LongContainer> FACTORY_MURMUR3HASH = (object, destination) ->
	{
	...
	final public static IDatabaseFactory<LongContainer, byte[]> FACTORY_DATABASE = new IDatabaseFactory<>()
	{
		@Override
		...
		@Override
		public @NullAllow byte[] toDatabase(@NullAllow LongContainer object)
		{
			if (object != null)
			{
				int count = object.size;
		...
		@Override
		public @NullAllow LongContainer fromDatabase(@NullAllow byte[] object)
			throws Exception
		{
			if (object != null)
			{
	...
	final public static Factories<LongContainer> FACTORIES = new Factories<>(FACTORY_INSTANCE, FACTORY_COPY, FACTORY_BYTE, FACTORY_MURMUR3HASH, FACTORY_DATABASE);


	//
	private int capacity;
	private int size;
	private @NullDisallow long[] array;


	//
	private LongContainer(@NullDisallow LongContainer source)
	{
	...
	public LongContainer(int capacity)
	{
	...
	public LongContainer(@NullDisallow long[] array)
	{
		this(array, false);
	}


	private LongContainer(@NullDisallow long[] array, boolean wrap)
	{
	...
	public long[] array(int from, int to)
	{
	...
	private int index0(long value, int from, int to)
	{
	...
	@Override
	public @NullDisallow Long get(int index)
	{
	...
	@Override
	public void add(@NullDisallow Long value)
	{
		add((long) value);
	}
	...
	public void add(LongContainer container)
	{
		add(container.array, 0, container.size);
	}


	public void add(LongContainer container, int from, int to)
	{
		Validator.argument(from >= 0, "[from] (", from, ") is illegal");
	...
	@Override
	public Factories<LongContainer> factories()
	{
		return FACTORIES;
	}
	...
	@Override
	public boolean equals(@NullAllow LongContainer object)
	{
		return
			this == object
			||
	...
	//
	public static LongContainer wrap(long[] array)
	{
		return new LongContainer(array, true);
	}
